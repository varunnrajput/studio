...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url.startsWith("/catalog")) {
    ...
    try {
      ...
      if (type === "anime") {
        // Cinemeta Anime Catalog (reliable, fast, no Cloudflare rate limits)
...
function buildSearchQueries(raw) {
  ...
  // 1. Sanitized query without punctuation (removes colons, hyphens that trigger MySQL NOT/operator syntax)
  add(raw);

  // 2. Primary title before hyphen, dash, or colon
...
async function searchApibayWithFallbacks(queryCandidates, limit) {
  ...
  for (const q of queryCandidates) {
    ...
    if (Array.isArray(items)) {
      ...
      // If we acquired adequate results, stop waterfall early
...
async function resolveMediaMeta(queryCandidates) {
  for (const q of queryCandidates) {
    ...
    if (candidates.length > 0) {
      // Prioritize official items over fan reactions, reviews, or #DUPE# entries
  ...
  if (req.url.startsWith("/search-torrents")) {
    const urlParams = new URL(req.url, `http://localhost:${PORT}`);
  ...
  if (req.url.startsWith("/api/")) {
    const targetUrl = new URL(`https://api.torbox.app/v1${req.url}`);
    const options = {
  ...
  // 4. Static File Serving (index.html, etc.)
  const parsedPath = req.url.split("?")[0];
  const filename = parsedPath === "/" ? "index.html" : parsedPath;
